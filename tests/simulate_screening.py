...
- ConversationStateMachine (Day 29) IS now driven directly, via the
  shared screening_ai.screening_pipeline.run_screening_pipeline()
  function. This replaces the original manual STT-status check with
  real retry/skip/follow-up flow control. A 5th candidate profile,
  noisy_mixed_language_candidate, was added specifically to exercise
  Day 31's noise_detected and language_mixed_detected STT statuses
  through the full pipeline for the first time — previously these were
  only unit-tested in isolation.

This script does not add to or modify the pytest suite. Full pytest suite
remains at 177 passed (Day 31 added 6 new tests on top of the Day 30 baseline of 171;
this script itself is not a pytest file and does not add to that count).
"""
...
def _noisy_mixed_language_qa_pairs() -> list[dict]:
    """Answers engineered to trigger Day 31's noise and language-mix
    detection paths in stt_processor.detect_audio_issue(), exercising
    those code paths end-to-end for the first time via the conversation
    state machine (previously only unit-tested in isolation)."""
    return [
        _qa_pair(0, "The FMEA process is good [inaudible] severity rating", 0.9),
        _qa_pair(1, "APQP requires ???? planning phases for new products", 0.9),
        _qa_pair(2, "मेरा अनुभव control chart variation tracking is strong", 0.9),
        _qa_pair(3, "CAPA root cause [background noise] analysis fishbone", 0.9),
        _qa_pair(4, "PPAP and 8D submission प्रक्रिया for customer complaints", 0.9),
    ]
...
def _build_candidate_profiles() -> list[tuple[str, str, list[dict[str, Any]]]]:
    ...
    return [
        ...
        ("noisy_mixed_language_candidate", JOB_ID, _noisy_mixed_language_qa_pairs()),
    ]
...
def run_candidate_simulation(
    ...
    Delegates entirely to screening_ai.screening_pipeline.run_screening_pipeline(),
    which drives the qa_pairs through the Day 29 ConversationStateMachine (real
    retry/skip/follow-up flow control) and, for every accepted answer, runs answer
    processing (Day 25), scoring (Day 26), and behavior analysis (Day 27). The
    pipeline's own report already includes "skipped_qa_pairs", "total_qa_pairs",
    and "behavior_reports" — this function only adds one further convenience
    field on top for this script's own summary table.
    ...
    Returns:
        The report dict from run_screening_pipeline(), augmented with one
        script-level field used only by this simulation's own summary table
        and calibration checks (not part of the pipeline's own report schema):
        "first_communication_strength" (the communication_strength of the
        first accepted answer, or None if every qa_pair was skipped).
    """
    # KEY DECISION — All STT cleaning, retry/skip/follow-up flow control, and
    # downstream answer/score/behavior processing is delegated entirely to
    # run_screening_pipeline(). Duplicating that logic here was the root of
    # the Day 29/30 integration gap: the old manual loop checked
    # stt_result["status"] != "processed" and skipped silently, bypassing
    # ConversationStateMachine's retry budget, skip-with-advance, and
    # follow-up branching entirely. The pipeline is now the single owner of
    # that flow contract; this function's only job is to call it and add the
    # simulation-level convenience field that _print_summary_table() reads.
    report = run_screening_pipeline(candidate_id, job_id, qa_pairs)

    # Script-level augmentation for the summary table / calibration checks below.
    # KEY DECISION — "behavior_reports" is already present in the report dict:
    # run_screening_pipeline() adds it explicitly in its Step 7 contract (see
    # screening_pipeline.py KEY DECISION (b) comment). Reading index [0] here
    # is safe because the list is non-empty check guards the access. This field
    # is not part of Day 28's report schema; it exists only for this script's
    # own _print_summary_table() and identify_threshold_issues() consumers.
    report["first_communication_strength"] = (
        report["behavior_reports"][0]["communication_strength"]
        if report["behavior_reports"]
        else None
    )
