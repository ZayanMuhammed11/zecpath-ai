...
# Day 24/31's stt_processor emits exactly these five status strings.
...
# instead of silently corrupting the interview history. Day 31 added
# "noise_detected" and "language_mixed_detected" alongside the original
# three Day 24 statuses — any other string still raises ValueError via
# the existing record_exchange() tripwire, and this is intentional and
# must not be relaxed as new status strings get added in the future.
VALID_STT_STATUSES = {
    "processed",
    "silence_detected",
    "poor_audio_detected",
    "noise_detected",
    "language_mixed_detected",
}
...
class ConversationStateMachine:
    ...
    def detect_issue(self, stt_status: str, answer_text: str) -> str:
        ...
        Args:
            stt_status: Status string from Day 24/31's stt_processor.
        ...
        Returns:
            ...
            "noise" if stt_status is "noise_detected",
            "language_mixed" if stt_status is "language_mixed_detected",
        ...
        if stt_status == "poor_audio_detected":
            return "poor_audio"
        # KEY DECISION: noise and language-mixing are new Day 31 STT-layer
        # issues, checked here right after the original Day 24 statuses and
        # before the too_short content check, since they describe a problem
        # with the audio/transcript itself rather than the answer's content.
        if stt_status == "noise_detected":
            return "noise"
        if stt_status == "language_mixed_detected":
    ...
    def get_retry_message(self, issue: str) -> str:
        ...
        Args:
            issue: One of "silence", "poor_audio", "noise", "language_mixed",
                "too_short", or any other value (falls back to a generic
                prompt).
        ...
        messages = {
            ...
            # KEY DECISION: Day 31 noise/language-mixed messages are phrased
            # as actionable requests (move somewhere quieter / respond in
            # English) rather than generic "please repeat" prompts, since
            # the underlying issue is different from silence/poor_audio and
            # a candidate can actually act on a more specific instruction.
            "noise": "There's some background noise, could you move somewhere quieter and repeat that?",
            "language_mixed": "Could you please respond in English so I can understand you clearly?",
        }
