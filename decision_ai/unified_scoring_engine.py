"""Deterministic unified scoring engine for Day 41 (extended Day 51).

Combines ATS, screening, HR, technical, and machine-test round scores
into a single unified hiring score with role-based weighting,
missing-round redistribution, hiring-fit categorization, a
recommendation, a confidence rating, and deterministic (non-LLM)
reasoning text.

No Redis, no LLM calls, no randomness — pure deterministic computation only.

Day 51 update: extended from 3 rounds (ats, screening, hr) to 5 rounds
(+ technical, machine_test) via _ROUND_FIELD_MAP. redistribute_weights(),
calculate_unified_score(), and generate_reasoning() already iterate over
_ROUND_FIELD_MAP generically and required no changes beyond that map
update. get_confidence() was rewritten (not just extended) to use
proportion-of-5 bands instead of a raw 3-round count — see its docstring
and DAY51_DECISIONS.md for details.
"""
...
def redistribute_weights(
    ...
    Args:
        ...
        base_weights: The base (pre-redistribution) weights for all
            rounds present in _ROUND_FIELD_MAP, keyed by round name
    ...
    Raises:
        ValueError: If all round scores are None.
    """
    present_rounds = [
        name
...
def get_confidence(round_scores: RoundScores) -> str:
    """Derive a confidence rating from the proportion of rounds present.

    Day 51 change (deliberate behavior change, NOT a bug fix): under the
    Day 41 3-round model this function mapped a raw round count directly
    to a band (3 -> high, 2 -> medium, 1 -> low). Now that there are 5
    possible rounds, a raw count is no longer a meaningful proxy for
    confidence on its own (e.g. 2 of 5 rounds present is much weaker
    signal than 2 of 3), so the bands have been redefined in terms of
    proportion of the 5 possible rounds:

        5 or 4 rounds present -> "high"
        3 or 2 rounds present -> "medium"
        1 round present       -> "low"

    This redefinition is driven purely by the increase from 3 to 5
    rounds and is documented as an intentional behavior change in
    DAY51_DECISIONS.md, not a defect fix.

    Args:
        round_scores: The candidate's raw round scores.

    Returns:
        "high", "medium", or "low" per the bands above. This function is
        standalone and does not assume it is only ever called after
...
def generate_reasoning(
    ...
    if breakdown.rounds_missing:
        ...
        missing_sentence = " All rounds were included with no redistribution needed."
...
def unified_scoring_pipeline(
    ...
    Raises:
        ValueError: If all round scores are None (propagated from
            redistribute_weights).
    """
