...
Each role level maps to a dict of base weights for the three rounds
(ats, screening, hr). These are the weights used before any missing-round
redistribution happens in unified_scoring_engine.redistribute_weights().
"""
...
ROLE_WEIGHTS: Dict[RoleLevel, Dict[str, float]] = {
    RoleLevel.fresher: {"ats": 0.25, "screening": 0.35, "hr": 0.40},
    RoleLevel.mid: {"ats": 0.30, "screening": 0.30, "hr": 0.40},
    RoleLevel.senior: {"ats": 0.35, "screening": 0.25, "hr": 0.40},
}
...
# (test_role_weights_sum_to_one) rather than relying on this assertion alone.
